...
function zurb_foundation_html_head_alter(&$head_elements) {
  ...
  $head_elements['description'] = array(
  	...
  	'#attributes' => array(
  	'name' => 'description',
  	'content' => 'Fortified Security Services has over thirty years experience installing and servicing burglar alarms, medical alarms, fire alarms, access control and camera systems throughout Arizona.  Our company holds a residential and commercial Low Voltage Communications License from the State of Arizona.',
  	),
  );
  ...
  $head_elements['keywords'] = array(
  	...
  	'#attributes' => array(
  	'name' => 'keywords',
  	'content' => 'Video Security, Security Cameras, Alarm Monitoring, Surveillance Systems, Cameras, Alarms, Guards, Virtual Guarding, Alarm Systems, Guard Service, Tucson Arizona,',
  	),
  );
...
/***function zurb_foundation_preprocess_node(&$vars) {
  ...
  //drupal_add_js(drupal_get_path('module', 'fr_virtual') .'/flowplayer.playlist-3.2.10.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 2));
...
function zurb_foundation_preprocess_user_login(&$vars) {
  $vars['intro_text'] = t('Customer Login:');
}

function zurb_foundation_preprocess_user_register_form(&$vars) {
  $vars['intro_text'] = t('');
}
...
function zurb_foundation_js_alter(&$js) {
  ...
  foreach ($js as $key => $js_script) {
 		...
 		if(array_key_exists('sites/all/modules/fr_virtual/flowplayer.playlist-3.2.10.js', $js)){
	     	$js[$key]['scope'] = 'header';
	  }else{

    }
    if($key == 'sites/all/modules/fr_user_dash/js/fr_user_dash_view_actions.js'){
	  	$js[$key]['scope'] = 'header';
  	}else{
